...
function TrashPage() {
  ...
  async function restore(row: any) {
    const { error } = await supabase.from("posts").update({ status: "draft" }).eq("id", row.original_post_id);
  ...
  return (
    <div>
      <PageHeader title="Riwayat Hapus" subtitle="Postingan yang dihapus dapat dipulihkan atau dihapus permanen" />
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {rows.length === 0 && <p className="col-span-full text-center text-sm text-muted-foreground">Trash kosong.</p>}
        {rows.map((r) => (
          <div key={r.id} className="panel rounded-2xl p-4">
            ...
            <div className="mt-3 flex gap-2">
              <Button size="sm" variant="secondary" onClick={() => restore(r)} disabled={r.restored}>
                <RotateCcw className="mr-1 h-3 w-3" />Restore
              </Button>
              <Button size="sm" variant="ghost" className="text-destructive" onClick={() => purge(r)}>
                <Trash2 className="mr-1 h-3 w-3" />Permanen
              </Button>
            </div>
          </div>
        ))}
