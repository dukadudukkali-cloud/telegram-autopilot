...
function HistoryPage() {
  ...
  async function repost(id: string) {
    ...
    if (r.ok) toast.success("Terkirim ulang");
  ...
  async function softDelete(id: string) {
    ...
    if (error) toast.error(error.message);
      ...
      toast.success("Dipindah ke Trash");
      load();
    }
  }
          ...
          <tbody className="divide-y divide-border/60">
            ...
            {rows.map((p) => (
              <tr key={p.id} className="hover:bg-muted/20">
                ...
                <td className="p-3">
                  <span className="rounded-full border border-border bg-muted/40 px-2 py-0.5 text-[10px] font-bold uppercase">
                    {p.status}
                  </span>
                </td>
                ...
                <td className="p-3 text-right">
                  <div className="inline-flex gap-1">
                    <Button asChild size="sm" variant="ghost">
                      <Link to="/posts/$postId/edit" params={{ postId: p.id }}>
                        Edit
                      </Link>
                    </Button>
                    <Button size="sm" variant="ghost" onClick={() => repost(p.id)}>
