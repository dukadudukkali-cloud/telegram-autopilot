...
function UsersPage() {
  ...
  async function changeRole(uid: string, role: "admin" | "operator") {
    try {
      await updateFn({ data: { targetUserId: uid, role } });
  ...
  async function changeStatus(uid: string, status: "active" | "inactive") {
    try {
      await updateFn({ data: { targetUserId: uid, status } });
  ...
  async function del(uid: string) {
    ...
    try {
      await deleteFn({ data: { targetUserId: uid } });
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <form onSubmit={submit} className="panel rounded-2xl p-6 lg:col-span-1">
          ...
          <div className="space-y-3">
            ...
            <Button disabled={busy} className="w-full glow">
              ...
              {rows.map((u) => (
                <tr key={u.user_id} className="hover:bg-muted/20">
                  ...
                  <td className="p-3">
                    <select
                      value={u.status}
                      onChange={(e) => changeStatus(u.user_id, e.target.value as any)}
                      className="h-8 rounded border border-input bg-background px-2 text-xs"
                    >
                      <option value="active">active</option>
                      <option value="inactive">inactive</option>
