      ...
      <div className="panel rounded-2xl p-6 max-w-2xl space-y-4">
        ...
        <div>
          ...
          <Input
            ...
            onChange={(e) => setS({ ...s, default_channel: e.target.value })}
          />
        </div>
        <div>
          <Label>Delay Posting (ms)</Label>
          <Input
            type="number"
            className="mt-1"
            value={s.posting_delay_ms}
            onChange={(e) => setS({ ...s, posting_delay_ms: Number(e.target.value) })}
          />
        </div>
        <div className="flex items-center justify-between rounded-lg border border-border p-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Coba sambungkan ulang otomatis jika gagal
            </p>
          </div>
          <Switch
            checked={s.auto_reconnect}
            onCheckedChange={(v) => setS({ ...s, auto_reconnect: v })}
          />
        </div>
        <div className="flex items-center justify-between rounded-lg border border-border p-3">
          <div>
            <Label>Dark Mode</Label>
            <p className="text-xs text-muted-foreground">Tema neon gelap (default)</p>
          </div>
