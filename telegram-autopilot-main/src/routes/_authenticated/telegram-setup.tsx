  ...
  async function runTest(accountId: string) {
    ...
    try {
      ...
      const r: any = await testFn({
        data: { configId: accountId },
        headers: {
          Authorization: `Bearer ${session?.access_token}`,
        },
      });
      // debug
      console.log("[telegram-setup] testFn response", r);

      if (r?.success) {
        toast.success("Bot berhasil terhubung", { id: tId });
      } else {
        const message =
          r?.message ||
          r?.description ||
          r?.error ||
          r?.telegram?.description ||
          "Terjadi error tanpa pesan dari server";
      ...
      const message =
        ...
        "Terjadi error tanpa pesan dari server";
      console.log("[telegram-setup] testFn error", {
        message,
        raw: e,
      });
      toast.error(message, { id: tId });
            ...
            <div className="md:col-span-2">
              ...
              <div className="mt-1 flex gap-2">
                <Input
                  ...
                  placeholder={
                    editingId
                      ? "Kosongkan untuk tetap memakai token tersimpan"
                      : "1234567890:ABC-DEF…"
                  }
                  value={botToken}
